 ...
 <div class="col-6">
            ...
            <div class="card card-outline card-primary">
              <div class="card-header">
                <h3 class="card-title"><b>Without Roll Number <?php echo $Sem; ?> Sem 
                ...
                &nbsp;&nbsp;
                <input class="btn bg-gradient-success" type="Button" onclick="Update_Roll('Add')" value='Submit'></b></h3>
              ...
              <div class="card-body table-responsive p-0" style="height: 400px;font-size:12px;">
                <table class="table table-head-fixed table-hover text-nowrap" id="mt1">
                  ...
                  <thead style="cursor: pointer;">
                    <tr>
                      	<th id="b1" onclick="sortTable('#b1')">RollNumber</th>
                      	<th id="b2" onclick="sortTable('#b2')">Name</th>
                      	<?php if($Program!=""){ ?>
  		      	<th id="b3" onclick="sortTable('#b3')">FName</th>
  			<?php }else{ ?>
  			<th id="b3" onclick="sortTable('#b3')">Branch</th>
  			<?php } ?>
                    
                       <th id="b4" onclick="sortTable('#b4')">Quota</th>
                    	<th id="b5" onclick="sortTable('#a5')">DOB</th>
			...
			echo "<tr>";
			   ...
			   while(true)
			   {
			     	$Rnew_roll=sprintf("%02d",$new_roll);
			     	$RRR=$Pre_Roll.$Rnew_roll;
			     
			     	$Add_sql="Select C_Roll_Number from Student_Info where C_Roll_Number='$RRR'";
				$Add_query= $dbh -> prepare($Add_sql);
				$Add_query-> execute();
			...
			?>
                      		...
                      		Check_Roll('<?php echo "NA".$Fres->Student_ID;?>','mt1')"/>
                      		</td>
  			...
  			<?php if($Program!=""){ ?>
  			<td ><?php echo $Fres->Father_Name  ; ?></td>
  			<?php }else{ ?>
          ...
          $Add_sql=" Select S1.Student_ID,
			...
			if($Section!="")
			{
				$Add_sql=$Add_sql." and S1.Section='$Section' ";
                       ...
                       $Add_sql=$Add_sql." order by S1.C_Roll_Number";
                       $Add_query= $dbh -> prepare($Add_sql);
			$Add_query-> execute();
    			$Fresult=$Add_query->fetchAll(PDO::FETCH_OBJ);
...
?>
 ...
 <div class="col-6">
             ...
             <div class="card card-outline card-success">
              <div class="card-header">
                <h3 class="card-title"><b>With Roll Number 	<?php echo $Sem; ?> Sem  
                 <?php if($Section!=""){ echo "Section:".$Section;}?>
                 <input class="btn bg-gradient-danger" type="Button" value='Clear' onclick="Clear_T('mt2')">&nbsp;&nbsp;
                <!--<input class="btn bg-gradient-warning" type="Reset"> -->
                &nbsp;&nbsp;<input class="btn bg-gradient-info" type="Button" onclick="Update_Roll('Update')" value='Update'> </b></h3>
              ...
              <div class="card-body table-responsive p-0" style="height: 400px;font-size:12px;">
                <table class="table table-head-fixed table-hover text-nowrap"  id="mt2">
                 ...
                 <thead style="cursor: pointer;">
                    <tr>
                    <th id="a1" onclick="sortTable('#a1')">RollNumber</th>
			...
			?>
                      	...
                      	<td> 
                      	<input  type="text"  class="lets"
                      	name="<?php echo "A".$Fres->Student_ID ?>"  value="<?php echo $Fres->C_Roll_Number ?>"
                      	id="<?php echo "A".$Fres->Student_ID ?>" 
                      	onkeyup="this.value = this.value.toUpperCase();
                      		Check_Roll('<?php echo "A".$Fres->Student_ID;?>','mt2')"/>
                      	</td>
