...
if(isset($_POST['C_USN']))
{
			...
			if($C_Roll_Number!="")
			{
    				$sql ="SELECT C_USN FROM Student_Info where C_Roll_Number=:C_Roll_Number1 ";
			...
			if($C_USN!="")
			{
                      		$sql ="SELECT C_USN FROM Student_Info where C_USN=:C_USN1";
    				$query= $dbh -> prepare($sql);
    			...
    			if($Praaptha_No!="")
    			{
    				$sql ="SELECT C_USN FROM Student_Info where Praaptha_No=:Praaptha_No1";
    				$query= $dbh -> prepare($sql);
    			...
    			if($Aadhaar!="")
    			{
    				$sql ="SELECT C_USN FROM Student_Info where Aadhaar=:Aadhaar1";
    				$query= $dbh -> prepare($sql);
    				$query-> bindParam(':Aadhaar1', $Aadhaar);
    				$query-> execute();
    				if($query->rowCount() > 0)
    				{
    				$Msg="Aadhaar Number/Paasport Number Alredy Exist";
    				}
    			}
    			echo $Msg;
